...
这两个模型不支持结构化输出，相关配置会返回 `400`。显式缓存可用：把要复用的长前缀放进 `system` 数组即可，写法与命中条件见[文本模型总览 · 缓存](/cn/api-reference/text/overview#缓存)。各模型支持的能力以[总览表](/cn/api-reference/text/overview#anthropic)为准。
