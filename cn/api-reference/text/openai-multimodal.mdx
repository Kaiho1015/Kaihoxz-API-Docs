...
- OpenAI Responses 协议入口，只服务声明了该协议的型号
- 已验收范围：文本非流式与流式输出
- 托管工具、图像与视频输入在本入口未开放
...
<ParamField body="model" type="string" required>
  ...
  **只有声明了 `openai_responses` 协议的型号走这个入口**，共五个：
  ...
  * `o3-pro`
  ...
  <Note>
  这五个型号**只**接受本接口，打到 `/v1/chat/completions` 会在预扣前拒绝；反过来，其余文本型号只走 Chat，打到本接口同样在预扣前拒绝。各型号价格见[文本模型总览](/cn/api-reference/text/overview)。
  </Note>
</ParamField>
...
<ParamField body="input" type="string or array" required>
  ...
  <Expandable title="消息对象属性">
    ...
    <ParamField body="content" type="array" required>
      内容数组，每项包含 `type` 字段。**本入口当前只验收了 `input_text`**（需提供 `text` 字段）；图像与视频输入见页尾「当前不开放」。
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="tools" type="array">
  ...
  <Warning>
  这五个型号在模型组里声明为**不支持工具调用与结构化输出**，因此 `tools`、`tool_choice`、`response_format` 都会在额度预扣前被拒绝（HTTP 400，不计费）。需要函数调用请用 [Chat 接口](/cn/api-reference/text/general-chat)上支持工具的型号。
  </Warning>
</ParamField>
...
`usage.input_tokens` 是输入总量（含 `input_tokens_details.cached_tokens`），`usage.output_tokens` 已包含 `output_tokens_details.reasoning_tokens`，思考 token 只按输出价计一次。这五个型号目前都是输入 / 输出两项计价，没有独立缓存价，响应里的缓存统计按普通输入价计。单价与计费通则见[文本模型总览 · 计费口径](/cn/api-reference/text/overview#计费口径)。
...
<Warning>
缺少可靠的终态用量时，已预扣额度会保留为待核对状态。先到控制台核对已有用量记录，不要自动重发。
</Warning>
...
- **托管工具** —— `web_search`、`file_search`、`remote_mcp` 等厂商内置工具，其额外收费维度尚未接入计费。
- **工具调用与结构化输出** —— 这五个型号声明为不支持，见上方 `tools` 说明。
- **`service_tier`** 取 `standard` / `default` 以外的值。

**图像与视频输入**当前没有在模型组里声明，也没有完成验收，因此本页不承诺可用；需要看图的场景请等待该能力正式声明后再接入。
