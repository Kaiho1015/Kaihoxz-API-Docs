...
<ParamField body="model" type="string" required>
  ...
  * `o3-pro`
  ...
  <Note>
  这五个模型只接受本接口，发到 `/v1/chat/completions` 会返回 `400`；其余文本模型只走 Chat，发到本接口同样返回 `400`。各模型的计费方式见[文本模型总览](/cn/api-reference/text/overview)。
  </Note>
</ParamField>
...
<ParamField body="tools" type="array">
  ...
  <Warning>
  这五个模型不支持工具调用与结构化输出，`tools`、`tool_choice`、`response_format` 都会返回 `400`，不计费。需要函数调用请在 [Chat 接口](/cn/api-reference/text/general-chat)上选支持工具的模型。
  </Warning>
</ParamField>
