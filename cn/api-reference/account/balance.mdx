...
- 本端点只返回你自己的数据，不含渠道、上游模型或 Key 明文等任何内部字段。

## 三个端点怎么选

| 想知道 | 用这个 |
|---|---|
| 还剩多少钱、用了多少、总共多少 | **本页**，一次拿全，推荐新接入用 |
| 只要累计已用（OpenAI 口径，单位美分） | [查询已用量](/cn/api-reference/account/usage) |
| 只要总额度（OpenAI 口径 `hard_limit_usd`） | [查询总额度](/cn/api-reference/account/subscription) |

后两个是为**兼容既有的 OpenAI 额度查询工具**保留的形状（用量看板、成本统计脚本等）。自己写代码时用本页这个更直观。
