---
title: "Kling 系列"
description: "快手可灵视频生成 — v2.6 / v3 / v3-omni / video-o1 / 3.0-turbo / 运动控制,覆盖文生、图生、首尾帧、多模态参考、视频参考与运动控制"
api: "POST https://www.qingbo.dev/v1/tasks"
---

快手可灵 Kling 视频生成系列,7 个模型分四类:

- **kling-v3** — 新一代基础模型,**质量档 720P / 1080P / 4K**,原生支持有声视频(`generate_audio`)
- **kling-v3-omni** — v3 系列**统一多模态接口**,文生 / 图生 / 视频参考全走同一 endpoint,prompt 支持 `<<<image_N>>>` 引用语法,支持有声与视频参考
- **kling-v2-6** — 经典稳定版,720P / 1080P,1080P 可有声,语义与动态稳定性打磨
- **kling-video-o1** — 推理增强视频模型,生成前对提示词与参考素材深度规划,物理一致性、复杂运动表现最优,支持视频参考
- **kling-3.0-turbo** — 极速档,普通分辨率 720P / 1080P,生成最快、单价最低,适合预览 / 批量
- **kling-v3-motion-control** / **kling-v2-6-motion-control** — **运动控制**:用一张参考图(角色)+ 一段参考视频(运动),让参考角色复刻参考视频的动作

## 质量档用 `resolution` 表达

kling v3 家族(v3 / v3-omni / video-o1)的价格按**质量模式**分档,我们对外**统一用 `resolution` 字段表达档位**,网关内部映射到 kling 的 `mode` 参数:

| `resolution` 取值 | 质量模式 | 网关内部 `mode` |
|---|---|---|
| `720P` | 标准模式 | `std` |
| `1080P` | 专业模式 | `pro` |
| `4K` | 4K 模式(仅 v3 / omni) | `4k` |

`kling-3.0-turbo` 与两个 `*-motion-control` 是**普通分辨率**含义(720P / 1080P),不涉及质量档映射。

按 **质量档 × 时长** 计费,**单位 `$/秒`**:实际计费 = 该档每秒单价 × `duration`。

**参考视频**(`video_urls`,`kling-v3-omni` / `kling-video-o1`):换成更贵的**参考视频档**单价,只乘 **输出秒数** —— 参考视频本身不计秒。

**运动控制**(`kling-v3-motion-control` / `kling-v2-6-motion-control`):按 **参考视频秒数 × 每秒档价** 计费,`720P` 按标准(std)档、`1080P` 按专业(pro)档;参考视频本身不另外计费。输出时长由参考视频决定,与参考片等长 —— 这两个模型**没有 `duration` 参数**。

## 定价

<Note>
本页不列单价 —— 价格会随上游调整,写在文档里迟早和实际对不上。

**看实时价格:** 控制台「模型市场」,或 `GET /v1/models` 查询接口(见 [模型列表](/cn/docs/models))。
**看单次实际花费:** 每次调用的响应里都带 `cost` / `usage.cost`,以它为准。
</Note>

## 调用示例

<CodeGroup>

```bash 文生视频(v3,有声)
curl -X POST https://www.qingbo.dev/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "kling-v3",
    "prompt": "一只柴犬穿宇航服在月球上行走,电影感光影",
    "duration": 5,
    "resolution": "1080P",
    "aspect_ratio": "16:9",
    "generate_audio": true
  }'
```

```bash 图生视频(首帧)
curl -X POST https://www.qingbo.dev/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "kling-v2-6",
    "prompt": "人物缓缓走向镜头,微风吹动头发",
    "image_urls": ["https://cdn.example.com/portrait.jpg"],
    "duration": 5,
    "resolution": "1080P"
  }'
```

```bash 4K 极致画质(v3)
curl -X POST https://www.qingbo.dev/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "kling-v3",
    "prompt": "航拍雪山日出,云海翻涌,壮阔史诗感",
    "duration": 5,
    "resolution": "4K",
    "aspect_ratio": "16:9"
  }'
```

```bash Omni 多模态参考(图)
curl -X POST https://www.qingbo.dev/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "kling-v3-omni",
    "prompt": "把 <<<image_1>>> 中的角色放到 <<<image_2>>> 的森林场景里奔跑",
    "image_urls": [
      "https://cdn.example.com/char.jpg",
      "https://cdn.example.com/forest.jpg"
    ],
    "duration": 5,
    "resolution": "1080P",
    "aspect_ratio": "16:9"
  }'
```

```bash O1 视频参考(推理增强)
curl -X POST https://www.qingbo.dev/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "kling-video-o1",
    "prompt": "保持 <<<image_1>>> 角色形象,沿用参考视频的镜头运动",
    "image_urls": ["https://cdn.example.com/char.jpg"],
    "video_urls": ["https://cdn.example.com/ref.mp4"],
    "duration": 10,
    "resolution": "1080P"
  }'
```

```bash 运动控制(参考图 + 参考视频)
curl -X POST https://www.qingbo.dev/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "kling-v3-motion-control",
    "prompt": "保持角色一致,跟随参考视频做转身挥手动作,电影感光影",
    "image_urls": ["https://cdn.example.com/character.jpg"],
    "video_urls": ["https://cdn.example.com/ref-motion.mp4"],
    "character_orientation": "video",
    "duration": 0,
    "resolution": "1080P"
  }'
```

</CodeGroup>

<ResponseExample>
```json 提交成功
{
  "task_id": "task-wave1775285160b950328499",
  "model": "kling-v3",
  "action": "generate",
  "status": "queued",
  "created_at": 1775285160040,
  "progress": 0
}
```
</ResponseExample>

提交后用 [`GET /v1/tasks/{task_id}`](/cn/api-reference/task/status) 轮询状态,详见 [任务系统](/cn/docs/task-system)。

## 可用模型

| 模型 ID | 分辨率 / 质量档 | 时长 | 有声 | 视频参考 | 特色 |
|---|---|---|---|---|---|
| `kling-v3` | 720P / 1080P / 4K | 5 / 10 秒 | ✅ `generate_audio` | — | 新一代基础模型,4K + 原生有声 |
| `kling-v3-omni` | 720P / 1080P / 4K | 5 / 10 秒 | ✅ `generate_audio` | ✅ `video_urls` | 统一多模态 endpoint |
| `kling-v2-6` | 720P / 1080P | 5 / 10 秒 | ✅ 仅 1080P | — | 经典稳定版 |
| `kling-video-o1` | 720P / 1080P | 5 / 10 秒 | — | ✅ `video_urls` | 推理增强,物理一致性最优 |
| `kling-3.0-turbo` | 720P / 1080P | 5 / 10 秒 | — | — | 极速档,生成最快、单价最低 |
| `kling-v3-motion-control` | 720P / 1080P | 跟随参考视频(3-30 秒) | — | 必填(运动源) | 运动控制(v3),无 `duration` 参数 |
| `kling-v2-6-motion-control` | 720P / 1080P | 跟随参考视频(3-30 秒) | — | 必填(运动源) | 运动控制(v2.6),无 `duration` 参数 |

所有模型均支持 `generate` action;`model` ID 即上表取值。

## 通用参数

<ParamField body="model" type="string" required>
  模型 ID,见 [可用模型](#可用模型) 表
</ParamField>

<ParamField body="prompt" type="string" required>
  视频描述文本。Omni / O1 中可用 `<<<image_N>>>` 语法引用 `image_urls` 第 N 张图(N 从 1 开始);运动控制档建议描述动作 / 镜头 / 风格
</ParamField>

<ParamField body="resolution" type="string" default="720P">
  输出分辨率 / 质量档。各模型支持范围见 [可用模型](#可用模型):
  - `720P` — v3 家族映射到标准模式(std)
  - `1080P` — v3 家族映射到专业模式(pro)
  - `4K` — v3 家族映射到 4K 模式(仅 `kling-v3` / `kling-v3-omni`)
</ParamField>

<ParamField body="duration" type="integer" default="5">
  视频时长(秒)。取值 `5` 或 `10`。

  两个 `*-motion-control` **不接受该参数** —— 输出时长由参考视频决定,调用时传 `0`(表示「跟随参考视频时长」)或不传
</ParamField>

<ParamField body="aspect_ratio" type="string" default="16:9">
  画面宽高比,**仅文生(T2V)生效**;图 / 视频参考模式跟随源素材比例。可选值:
  - `16:9` — 横版宽屏
  - `9:16` — 竖版长屏
  - `1:1` — 正方形
</ParamField>

<ParamField body="image_urls" type="string[]">
  参考图片 URL 数组:
  - **图生 / 首尾帧**(v3 / v2.6 / o1 / 3.0-turbo):传 1 张作首帧;v3 / omni / o1 传 2 张自动作首帧 + 尾帧
  - **多模态参考**(omni / o1):多张图,可被 `<<<image_N>>>` 引用
  - **运动控制**(两个 `*-motion-control`):**必填**,1 张作角色参考图
</ParamField>

<ParamField body="video_urls" type="string[]">
  参考视频 URL 数组(单元素):
  - **视频参考**(omni / o1):一段参考视频,沿用其镜头 / 运动特征
  - **运动控制**(两个 `*-motion-control`):**必填**,运动来源视频,让参考角色复刻其动作。**缺少时提交即被拒,不预扣。**
</ParamField>

<ParamField body="seed" type="integer">
  随机种子,固定可复现结果
</ParamField>

<ParamField body="callback_url" type="string">
  Webhook 回调地址,任务终态时调用。详见 [回调机制](/cn/docs/sync-async#异步任务-webhook)
</ParamField>

<ParamField body="callback_events" type="string[]">
  需要推送的事件类型,默认终态全推
</ParamField>

## 模型特定参数

<Tabs>

<Tab title="kling-v3">

<ParamField body="generate_audio" type="boolean" default="false">
  是否生成有声视频。各质量档均支持原生有声;开启后切换到对应「有声」单价(见 [定价](#定价))
</ParamField>

</Tab>

<Tab title="kling-v3-omni">

<ParamField body="generate_audio" type="boolean" default="false">
  是否生成有声视频。**与 `video_urls` 互斥** — 挂视频参考时不可开
</ParamField>

<ParamField body="video_urls" type="string[]">
  参考视频(单元素)。挂上后按「视频参考」档计费,且 `generate_audio` 不可同时开启
</ParamField>

</Tab>

<Tab title="kling-v2-6">

<ParamField body="generate_audio" type="boolean" default="false">
  是否生成有声视频。**仅 `resolution=1080P` 支持**;720P 不可开
</ParamField>

</Tab>

<Tab title="kling-video-o1">

<ParamField body="video_urls" type="string[]">
  参考视频(单元素)。挂上后按「视频参考」档计费。O1 不支持有声
</ParamField>

</Tab>

<Tab title="kling-3.0-turbo">

极速档无特定参数,所有可选项见 [通用参数](#通用参数)。不支持有声与视频参考。

</Tab>

<Tab title="motion-control">

`kling-v3-motion-control` / `kling-v2-6-motion-control` 必须同时提供:

<ParamField body="image_urls" type="string[]" required>
  角色参考图(1 张),**必填**,决定生成视频中的角色形象。**缺少时提交即被拒,不预扣。**
</ParamField>

<ParamField body="video_urls" type="string[]" required>
  运动来源视频(1 段),**必填**,生成视频会让参考角色复刻这段视频的动作 / 镜头。长度 **3-30 秒**。**缺少时提交即被拒,不预扣。**
</ParamField>

<Note>
运动控制要求**参考图与参考视频同时存在**;参考视频 MP4 / MOV,3-30 秒。

**没有 `duration` 参数**(上游不接受):输出时长由参考视频决定,与参考片等长,调用时 `duration` 传 `0` 或不传。计费按 **参考视频秒数 × 每秒档价**(`720P` 标准档 / `1080P` 专业档)。
</Note>

</Tab>

</Tabs>

## 特殊语法说明

**`<<<image_N>>>` 提示词图片引用**(Omni / O1 适用)

在 `prompt` 中用 `<<<image_N>>>` 占位符显式引用 `image_urls` 数组中的第 N 张图(N 从 **1** 开始)。模型会把占位符替换为对应图像内容,用于:

- 角色一致性 — `<<<image_1>>>` 锁定主体形象
- 场景拼合 — `<<<image_1>>>` 中的角色出现在 `<<<image_2>>>` 的环境
- 多素材引导 — 任意位置嵌入 N 张参考图

```text 示例
"把 <<<image_1>>> 中穿红衣的女孩,放到 <<<image_2>>> 的雪山场景里,镜头缓慢推进"
```

对应 `image_urls`:

```json
"image_urls": [
  "https://cdn.example.com/girl-red.jpg",
  "https://cdn.example.com/snow-mountain.jpg"
]
```

<Note>
Omni / O1 提供了图时若 `prompt` 未含任何 `<<<image_N>>>`,系统会自动在开头补 `<<<image_1>>>`。非 Omni / O1 的 v2.6 / v3 / 3.0-turbo 不解析该占位符,会作为字面文本送入模型。
</Note>

## 资源限制

| 项目 | 限制 |
|---|---|
| 参考图片(单张) | ≤ 30MB,JPG / PNG / WEBP |
| 参考图片(数量) | 图生 / 首尾帧:1-2 张;Omni / O1 多模态:多张;运动控制:1 张 |
| 参考视频(`video_urls`) | MP4 / MOV,运动控制 3-30 秒且 ≤ 100MB;视频参考 ≥ 3 秒、分辨率 720px-2160px |
| 提示词 | ≤ 2500 字符 |
| 输出 | MP4,链接 24 小时有效 |

## 相关文档

- [任务系统详解](/cn/docs/task-system) — 任务状态机 / 轮询节奏 / 异步推送
- [请求响应格式](/cn/docs/request-response) — 通用错误码 / Headers / 限流
- [认证](/cn/docs/authentication) — API Key 申请与使用
