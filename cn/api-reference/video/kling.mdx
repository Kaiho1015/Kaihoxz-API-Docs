...
**运动控制**(`kling-v3-motion-control` / `kling-v2-6-motion-control`):输出时长 **等于参考视频时长**,参考视频不另计费;`720P` 按标准(std)档、`1080P` 按专业(pro)档单价计。
...
<ParamField body="video_urls" type="string[]" required>
  运动来源视频(1 段),**必填**,生成视频会让参考角色复刻这段视频的动作 / 镜头。**缺少时提交即被拒,不预扣。**
</ParamField>
