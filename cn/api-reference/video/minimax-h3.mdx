...
按 **分辨率 × 时长** 计费,`2k` 档更贵。**传参考视频(`video_urls`)时,参考视频秒数与输出秒数叠加后按同一档单价计费**(不换档、不翻倍),`minimax-h3-regeneration` 同此口径。参考视频最长 **15 秒**。参考图前 5 张免费,之后上游按张加收,我们目前**不另计**。
...
<ParamField body="video_urls" type="string[]">
  参考视频,提供运镜、节奏与风格参考。最长 **15 秒**;`minimax-h3-regeneration` **必传**,**缺少时提交即被拒,不预扣。**
</ParamField>
