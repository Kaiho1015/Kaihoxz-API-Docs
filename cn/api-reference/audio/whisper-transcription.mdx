...
| 模型 ID | 说明 |
|---------|------|
| `gpt-4o-transcribe` | OpenAI 当代语音转写,词错误率明显低于 Whisper,对口音与嘈杂音频更稳 |
| `gpt-4o-mini-transcribe` | 经济档,价格是 gpt-4o-transcribe 的一半,准确率仍优于 Whisper |
| `whisper-1` | OpenAI Whisper,支持多语言识别;三者按音频分钟数计费。**单价高于上面两档**,新接入建议优先用 `gpt-4o-transcribe` / `gpt-4o-mini-transcribe` |
...
<ParamField body="model" type="string" required>
  模型 ID:`gpt-4o-transcribe` / `gpt-4o-mini-transcribe` / `whisper-1`。翻译端点(`/v1/audio/translations`)目前仅 `whisper-1` 支持
</ParamField>
