...
输出 token 的量级差异很大：同一画幅下，`high` 档的预计输出 token 约是 `low` 档的 **36 倍**，`4k` 又比 `1k` 高数倍，两者叠加后单张成本可以差两个数量级。**画质档是这条线路最大的成本变量**，不确定时先用 `low` 试。横幅比方图省，同档下 `16:9` 的输出 token 明显低于 `1:1`。
