...
逆向线路不支持 `quality`、`mask_url`、`background`、`output_format`、`output_compression`、`moderation` 和 `seed`，这些是官方线路独有的参数。本页未列出的参数会返回 `400`，不计费。
...
分辨率不在上述三档内时，按 `1k` 的价格结算。只有成功出图才计费，任务失败、取消，或没有返回可用图片时全额退款。
