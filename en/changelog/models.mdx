...
- Replaces the delisted `tts-1` / `tts-1-hd`. Eleven voices (`alloy` / `ash` / `ballad` / `coral` / `echo` / `fable` / `onyx` / `nova` / `sage` / `shimmer` / `verse`), plus a new `instructions` parameter for setting tone, pace and emotion in plain language.
...
**seedance-2.0-fast now accepts reference videos**

- `seedance-2.0-fast` now supports `video_urls` (reference video), matching `seedance-2.0` and `seedance-2.0-mini`.
- Billed as (reference-video seconds + output seconds) × the video-reference rate; a reference video may be at most 15 seconds.
- **Docs**: [Seedance series](/en/api-reference/video/seedance)
</Update>
...
- **Docs**: [Gemini image models](/en/api-reference/image/gemini)
</Update>
...
- **Omitting `quality` bills at the `high` rate** — see [GPT-Image series](/en/api-reference/image/gpt-image).
