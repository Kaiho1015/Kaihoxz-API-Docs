...
- **Tier differences**: the flagship charges \$0.0375 at both `1k` and `2k`; the fast tier is \$0.01875 at `1k` and \$0.025 at `2k` and adds the extreme ratios `1:4`, `4:1`, `1:8` and `8:1`; the lightest is \$0.015626, always outputs 1K and rejects `resolution`.
...
- **The reverse line** accepts none of those line-specific parameters and takes up to 15 reference images. It bills a **fixed price per image by output resolution** (1K \$0.0095625 / 2K \$0.01575 / 4K \$0.023625), so the cost of a call is known before you send it.
- The two lines never switch automatically — pin the model ID in your configuration as a product choice.
- **Docs**: [GPT Image family](/en/api-reference/image/gpt-image/overview) · [Official line](/en/api-reference/image/gpt-image/gpt-image-2) · [Reverse line](/en/api-reference/image/gpt-image/gpt-image-2-rev)
</Update>
