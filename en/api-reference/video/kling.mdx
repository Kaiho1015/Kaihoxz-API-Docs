...
<ParamField body="video_urls" type="string[]">
  Reference video URL array (single element). Omni / O1 — the clip supplies the motion and style reference. For the two `*-motion-control` models it is **required** as the motion source — If it is missing, the request is rejected at submission and nothing is held.
</ParamField>
...
| Reference video (`video_urls`) | MP4 / MOV, ≤ 100MB, 2-30 sec, **at most 1 clip** |
