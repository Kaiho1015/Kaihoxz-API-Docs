...
Billed by **resolution × duration**; `2k` costs more. **With a reference video (`video_urls`), reference seconds and output seconds are added together and billed at the same tier** — no tier switch, no doubling; `minimax-h3-regeneration` follows the same rule. A reference video may be at most **15 seconds**. The first 5 reference images are free; beyond that the upstream charges per image, which we currently **do not** pass on.
...
<ParamField body="video_urls" type="string[]">
  Reference video — supplies camera movement, pacing and style. At most **15 seconds**; **required** for `minimax-h3-regeneration` — If it is missing, the request is rejected at submission and nothing is held.
</ParamField>
