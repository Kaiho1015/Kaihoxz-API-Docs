...
**Reference video surcharge.** Passing `video_urls` moves the call to the pricier **video-reference tier** (the upstream's "uploaded videos" tier); without it you pay the ordinary tier for that resolution. Either way the price is multiplied by **output seconds only** — the reference clip itself is not billed by the second. Fast and std each have their own video-reference rates.
...
<ParamField body="video_urls" type="string[]">
  ...
  <Warning>
  When you pass reference videos, the prompt **must reference them with `@video_1` / `@video_2`** (e.g. `follows the motion in @video_1`). Without that reference the upstream rejects the request with `Reference video tag @video_1 must be in prompt`, and the request is not billed.
  </Warning>
</ParamField>
