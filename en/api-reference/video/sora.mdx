...
<Warning>
**Upstream shutdown notice**: OpenAI has announced that the Sora 2 models and the Videos API shut down on **2026-09-24**. `sora-2` / `sora-2-pro` are delisted here the same day; tasks submitted afterwards are rejected and not charged. Alternatives are listed in [Service Notices](/en/changelog/notices).
</Warning>
