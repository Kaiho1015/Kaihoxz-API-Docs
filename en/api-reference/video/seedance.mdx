...
The one exception is a **reference video** (`video_urls`): the upstream charges for input-video seconds plus output seconds, which we currently approximate as **2× the unit price** per output second. When the reference video is much longer than the output, actual cost can exceed this — it is an interim rule and will move to exact input-duration billing.
...
| `seedance-2.0-fast` | 480p / 720p | any integer 4-15 | Fast tier, faster and cheaper |
| `seedance-2.0-mini` | 480p / 720p | any integer 4-15 | Mini tier, lowest cost, same capabilities as standard |
