...
| `+ video_urls` | **reference_video** | Video clip reference (`2.0` / `2.0-fast` / `2.0-mini`) |
...
<ParamField body="video_urls" type="string[]">
  Array of reference-video URLs (`2.0` / `2.0-fast` / `2.0-mini`), up to 3 clips, each at most **15 seconds** (30 seconds for `seedance-2.5`); triggers **reference_video** mode. **Billing uses the video-reference tier in place of the base price** (cheaper than pure generation), applied to reference seconds plus output seconds
</ParamField>
...
| Reference videos (`video_urls`) | `2.0` / `2.0-fast` / `2.0-mini`, up to 3 clips, ≤ 15 sec each (30 sec for `2.5`), MP4 / MOV, ≤ 100MB per clip |
