...
<ResponseField name="result" type="object">
  ...
  - `expires_at` — Unix timestamp in seconds; a suggested download deadline. It is recomputed **on every query** as "now + 3 days" and is not a storage-side retention promise
</ResponseField>
...
<Warning>
**Download and persist result links to your own storage promptly.** `expires_at` is a suggested download deadline of 3 days from the moment of the query and is recomputed on every query; it neither guarantees that the file is kept until then nor that it disappears afterwards. Do not rely on result links for long-term storage.
</Warning>
