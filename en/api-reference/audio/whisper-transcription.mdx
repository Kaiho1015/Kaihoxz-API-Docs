...
|---------|------|
| `gpt-4o-transcribe` | OpenAI's current-generation transcription — markedly lower word-error rate than Whisper, more robust to accents and noisy audio |
| `gpt-4o-mini-transcribe` | Budget tier at half the price of gpt-4o-transcribe, still ahead of Whisper on accuracy |
| `whisper-1` | OpenAI Whisper, multilingual recognition; all three are billed per minute of audio. **Priced above both tiers above** — prefer `gpt-4o-transcribe` / `gpt-4o-mini-transcribe` for new integrations |
...
<ParamField body="model" type="string" required>
  Model ID: `gpt-4o-transcribe` / `gpt-4o-mini-transcribe` / `whisper-1`. The translation endpoint (`/v1/audio/translations`) currently supports `whisper-1` only
</ParamField>
