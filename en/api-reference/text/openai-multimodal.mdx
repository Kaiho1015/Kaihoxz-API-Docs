...
- The OpenAI Responses protocol entry, serving only the models that declare it
- Accepted scope: non-streaming and streaming text output
- Hosted tools, image input and video input are not open on this entry
...
<ParamField body="model" type="string" required>
  ...
  **Only models declaring the `openai_responses` protocol are served here** — five in total:
  ...
  * `o3-pro`
  ...
  <Note>
  These five accept **only** this endpoint and are rejected before reservation on `/v1/chat/completions`; conversely, every other text model is Chat-only and is rejected before reservation here. Prices are in the [Text Models Overview](/en/api-reference/text/overview).
  </Note>
</ParamField>
...
<ParamField body="input" type="string or array" required>
  ...
  <Expandable title="Message object properties">
    ...
    <ParamField body="content" type="array" required>
      Array of content parts, each with a `type`. **Only `input_text` (with a `text` field) is accepted on this entry today**; image and video input are covered under "Not available" below.
    </ParamField>
  </Expandable>
</ParamField>
...
<ParamField body="tools" type="array">
  ...
  <Warning>
  These five models are declared in their model groups as **not supporting tool calling or structured output**, so `tools`, `tool_choice` and `response_format` are rejected before quota is reserved (HTTP 400, no charge). For function calling, use a tool-capable model on the [Chat API](/en/api-reference/text/general-chat).
  </Warning>
</ParamField>
...
<Warning>
Without reliable terminal usage the reservation is held pending reconciliation. Check the existing usage record in the console before resubmitting; never retry automatically.
</Warning>
...
- **Hosted tools** — `web_search`, `file_search`, `remote_mcp` and other vendor built-ins, whose extra charge dimensions are not wired into billing.
- **Tool calling and structured output** — declared unsupported on these five models, see the `tools` note above.
- **`service_tier`** set to anything other than `standard` / `default`.

**Image and video input** are not declared on any model group and have not been through acceptance, so this page makes no promise about them; wait until the capability is declared before building on it.

## Related

<CardGroup cols={2}>
  <Card title="Text Models Overview" icon="table-list" href="/en/api-reference/text/overview">
    Prices, tiers and billing rules for the five models
  </Card>
  <Card title="General Chat API" icon="comments" href="/en/api-reference/text/general-chat">
    The default entry for every other text model
  </Card>
</CardGroup>
