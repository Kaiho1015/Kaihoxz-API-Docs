...
Neither model supports structured output; sending that configuration returns `400`. Explicit `cache_control` is supported: put the reusable long prefix in the `system` array — request shape and hit conditions are in the [Text Models Overview · Caching](/en/api-reference/text/overview#caching). What each model supports is in the [overview table](/en/api-reference/text/overview#anthropic). Cache TTLs, context limits, native tool workflows and multimodal capabilities are verified per model against the scope shown in the overview table — official support does not make a capability accepted on this route.
