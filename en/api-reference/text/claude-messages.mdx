...
<ParamField body="model" type="string" required>
  ...
  The native envelope currently carries Anthropic models only. Native non-streaming and streaming text is verified for `claude-sonnet-5` and `claude-opus-5`; prices and verified capabilities for the rest of the Claude line are in the [Text Models Overview · Anthropic](/en/api-reference/text/overview#anthropic).
...
Structured-output configuration is rejected before quota reservation, and this endpoint does not promise full compatibility with every Claude parameter. Cache TTLs, context limits, native tool workflows and multimodal capabilities are verified per model against the scope shown in the overview table — official support does not make a capability accepted on this route.
