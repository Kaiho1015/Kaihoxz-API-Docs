...
Each model serves only the protocols it supports (Chat / Responses / Claude Messages / Gemini native). Sending a request to a protocol a model does not serve, or sending `tools`, `tool_choice` or `response_format` to a model that does not support them, also returns `400`. What each model supports is in the catalog above.
...
- `gpt-5.6-luna` — 272K long-context tier; has a separate cache-write rate; max input 922K, max output 128K; supports automatic cache hits; supports automatic cache writes and hits; the long-context tier applies above 272K
...
- `claude-fable-5-1` — cache writes are priced at two TTLs, 5-minute and 1-hour; max input 1M, max output 128K; JSON Schema structured output is not supported; `tool_choice` accepts only `"auto"`; `"required"` and named functions return 400; cache hits are decided upstream and are not guaranteed
- `claude-fable-5` — cache writes are priced at two TTLs, 5-minute and 1-hour; max input 1M, max output 128K; JSON Schema structured output is not supported; `tool_choice` accepts only `"auto"`; cache hits are decided upstream and are not guaranteed
- `claude-opus-5` — cache writes are priced at two TTLs, 5-minute and 1-hour; max input 128K, max output 16K; JSON Schema and `json_object` are not supported; the native `/v1/messages` endpoint is available; cache hits are decided upstream and are not guaranteed
- `claude-sonnet-5` — cache writes are priced at two TTLs, 5-minute and 1-hour; max input 1M, max output 128K; JSON Schema and `json_object` are not supported; the native `/v1/messages` endpoint is available; cache hits are decided upstream and are not guaranteed

Use `max_tokens` on the Chat endpoint; do not send the GPT-only `reasoning_effort` / `max_completion_tokens`. The four Claude 5 / Fable models carry three cache rates (read / 5-minute write / 1-hour write) and are the only models that currently **allow explicit `cache_control`** — see [Caching](#caching) for the request shape. These four do **not support structured output**: `response_format` with `json_schema` or `json_object` returns 400. On the Fable models `tool_choice` accepts only `"auto"`; `"required"` or a named function returns 400. The 4.x line bills input and output only and does not support `cache_control`. Apps built on the Anthropic SDK can use the native [Messages endpoint](/en/api-reference/text/claude-messages), but **only `claude-sonnet-5` and `claude-opus-5` declare the `anthropic_messages` protocol**; any other Claude model returns 400 there — use the Chat endpoint instead.
...
- `gemini-3.8-flash` — max input 1,048,576, max output 65,536; the Gemini native endpoint is available; implicit cache-read rate; hits decided upstream
- `gemini-3.7-flash` — max input 1,048,576, max output 65,536; the Gemini native endpoint is available; implicit cache-read rate; hits decided upstream
- `gemini-3.6-flash` — max input 1,048,576, max output 65,536; the Gemini native endpoint is available; cache hits are decided upstream and are not guaranteed
...
- `deepseek-v4-pro` — max input 1M, max output 393K; time-of-day pricing, see the table at the end of this section; supports automatic cache hits
- `deepseek-v4-flash` — max input 128K, max output 16K; time-of-day pricing, see the table at the end of this section; cache hits are decided upstream and are not guaranteed
...
V4 Pro / Flash are priced by the **UTC time slot at request start** (Beijing time = UTC+8). The rate is fixed when the request begins and is used for both the response cost and the final settlement, even if the response crosses a slot boundary. The examples pass `thinking: {"type": "disabled"}` to turn thinking off. V4 cache-hit rates replace the matching input charge; there is no separate cache write. V3.x / R1 bill two rates; `deepseek-v3.2-exp` and `deepseek-v3.1-terminus` offer no strict JSON Schema — use `deepseek-v3.2` or `deepseek-r1-0528` when you need schema guarantees.
...
- `qwen3.8-max` — max input 983K, max output 131K; supports automatic cache hits; supports automatic caching, with hits decided upstream; explicit `cache_control` is not supported; use `tool_choice: "auto"`
