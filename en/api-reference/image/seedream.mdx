...
ByteDance Seed team's image generation series — **two generations, four tiers**:

- **5.0 Pro** — Quality-first: cinematic detail and text accuracy, 1K / 1.5K / 2K (1.5K costs the same as 1K and is sharper), up to 10 reference images, 2K priced separately
- **5.0 Lite** — Visual reasoning (CoT) + real-time web search, strong identity preservation (eyes / jawline / proportions / skin tone hold across styles), 2K / 3K / 4K, the value tier
- **4.5** — The refined 4.x tier: better text accuracy and multi-reference style fusion, 2K / 4K, up to 10 reference images and 15 images per request
- **4.0** — Unified text-to-image and editing, strong in-image text rendering, 1K / 2K / 4K, up to 10 reference images; a generation behind 5.0 and priced lower
...
|---|---|
| `seedream-5.0-pro` | Quality-first tier, cinematic detail, 1K / 1.5K / 2K (1.5K same price as 1K; 2K priced separately; omitting `resolution` bills at 2K), up to 10 reference images |
| `seedream-5.0-lite` | Visual reasoning + web search, strong identity preservation, 2K / 3K / 4K, unified generate / edit / image-set interface |
| `seedream-4.5` | Refined 4.x tier, better text accuracy and multi-reference fusion, 2K / 4K (no 1K) |
| `seedream-4.0` | Unified generation + editing, strong text rendering, 1K / 2K / 4K, lowest price |
...
<ParamField body="resolution" type="string" default="2K">
  ...
  `seedream-4.5` does not support `1K`; the request is rejected.
</ParamField>
