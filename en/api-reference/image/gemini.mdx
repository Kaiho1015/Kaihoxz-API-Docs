...
- **Nano Banana 2 Lite (Gemini 3.1 Flash-Lite Image)** — fastest and cheapest tier, `1K` only, ~4 s per image, built for high-volume generation
...
curl -X POST https://www.qingbo.dev/v1/tasks \
  ...
  -d '{
    "model": "gemini-3-pro-image-preview-rev",
...
curl -X POST https://www.qingbo.dev/v1/tasks \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "model": "gemini-2.5-flash-image-preview-rev",
    "action": "edit",
...
curl -X POST https://www.qingbo.dev/v1/tasks \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "model": "gemini-3.1-flash-image-preview-rev",
...
|---|---|---|
| `gemini-3-pro-image-preview` | Nano Banana Pro · Official Direct | Flagship tier, official direct, higher stability |
| `gemini-3-pro-image-preview-rev` | Nano Banana Pro · Standard | Flagship tier, native 4K, professional fidelity, multi-step planning |
| `gemini-3.1-flash-image-preview` | Nano Banana 2 · Official Direct | Fast tier with direct channel, search augmentation |
| `gemini-3.1-flash-image-preview-rev` | Nano Banana 2 · Standard | Fast tier with Google text + image search augmentation |
| `gemini-3.1-flash-lite-image-rev` | Nano Banana 2 Lite · Standard | Fastest, cheapest tier, `1K` only, up to 14 reference images |
| `gemini-2.5-flash-image-preview` | Nano Banana · Official Direct | Previous-gen direct, high cost-performance |
| `gemini-2.5-flash-image-preview-rev` | Nano Banana · Standard | Previous-gen workhorse, low latency, multi-image fusion |
...
<ParamField body="resolution" type="string" default="1K">
  ...
  - 3 Pro: `1K` / `2K` / `4K`
  - 3.1 Flash: `0.5K` / `1K` / `2K` / `4K` (the standard tier `gemini-3.1-flash-image-preview-rev` does not support `0.5K`; it starts at `1K`)
  - 2.5 Flash: `1K`
</ParamField>
...
<Tabs>
  ...
  <Tab title="3 Pro">
    **model id**: `gemini-3-pro-image-preview-rev`
  ...
  <Tab title="3.1 Flash">
    **model id**: `gemini-3.1-flash-image-preview-rev`

    **Series**: Nano Banana 2 · Standard

    **Supported actions**: `generate` / `edit`

    **resolution**: `1K` / `2K` / `4K` (the standard tier does not support `0.5K`)

    **aspect_ratio**: 11 options (including `auto`)

    **Highlights**: Lightweight fast tier — low latency, low cost, from `1K` up to `4K`, ideal for previewing, batches, and iteration. **Exclusive support for Google text and image search augmentation**, generating images grounded in real-time web information.
  ...
  <Tab title="3.1 Flash-Lite">
    **model id**: `gemini-3.1-flash-lite-image-rev`

    **Series**: Nano Banana 2 Lite · Standard

    **Supported actions**: `generate` / `edit`

    **resolution**: `1K` only (`2K` / `4K` / `0.5K` are silently downgraded to `1K`, no error)

    **aspect_ratio**: 11 options (including `auto`)

    **Reference images**: up to 14

    **Highlights**: The fastest, cheapest image model in the Gemini 3.1 family — text-to-image in about 4 seconds, built for high-volume, low-cost generation and rapid visual exploration. **No** `google_search` / `google_image_search` grounding, no `mask_url` inpainting, and no official-direct tier. All outputs carry Google's invisible SynthID watermark.
  </Tab>
  ...
  <Tab title="2.5 Flash">
    **model id**: `gemini-2.5-flash-image-preview-rev`
