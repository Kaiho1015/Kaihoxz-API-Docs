...
<CardGroup cols={2}>
  <Card title="Nano Banana Economy" href="/en/api-reference/image/gemini/nano-banana-rev">
    Google Gemini Image — Pro / 2 / 2 Lite economy routes, fixed per-image pricing, up to 14 reference images
  </Card>
  <Card title="GPT Image" href="/en/api-reference/image/gpt-image/overview">
    GPT Image family — official and reverse GPT Image 2 lines through one task endpoint, selected by model ID
  </Card>
...
Images have two entries. **Billing is the same on both** (settled from the model group's price config); they differ in validation and how results come back:
...
| Validation | **Full** — action, aspect ratio, resolution, quality, reference-image count and each model's `constraints` are checked **before any quota is reserved**; a non-conforming request is rejected and not charged | An OpenAI compatibility layer that does **no model-group capability validation**: invalid parameters are not caught up front, they are passed upstream as-is |
...
| **Inpainting (with mask)** | `+ image_urls + mask_url + action: edit` | [`gpt-image-2`](/en/api-reference/image/gpt-image/gpt-image-2) (official line) only; **not supported** on the Nano Banana economy routes |
...
- **Mask field** `mask_url` (used for `gpt-image-2` inpainting)
