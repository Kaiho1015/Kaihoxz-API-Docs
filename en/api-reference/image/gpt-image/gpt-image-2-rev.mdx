...
<ParamField body="n" type="integer" default="1">
  Fixed at `1`; each task generates one image.
</ParamField>

This line does not accept `quality`, `mask_url`, `background`, `output_format`, `output_compression`, `moderation`, or `seed`. Undeclared parameters are rejected before submission.
...
`generate` and `edit` use the same price, and reference images do not add a fee. Failed, cancelled, and zero-output tasks are fully refunded. Check the model catalog for the current quote; the completed task's `cost` is authoritative.
