---
title: "GPT Image Series"
description: "The official and reverse lines of GPT Image 2: one set of request parameters, two billing models"
---

GPT Image 2 comes in an official and a reverse line, both served on `POST /v1/tasks` — the request URL does not change between them. The parameters are largely the same; the differences are line-specific parameters and billing. Pick the model ID that fits.

## Available models

<CardGroup cols={2}>
  <Card title="GPT Image 2 Official" href="/en/api-reference/image/gpt-image/gpt-image-2">
    `gpt-image-2` · text-to-image, edits with up to 16 references, mask inpainting, transparency, and output controls; billed on actual token usage
  </Card>
  <Card title="GPT Image 2 Reverse" href="/en/api-reference/image/gpt-image/gpt-image-2-rev">
    `gpt-image-2-rev` · text-to-image and edits with up to 15 references; fixed per-image pricing at 1K, 2K, or 4K
  </Card>
</CardGroup>

## Choose a line

| Need | Recommended model | Why |
|---|---|---|
| Masks, transparent backgrounds, quality tiers, or output-format controls | `gpt-image-2` | The official line exposes the full parameter set |
| A fixed per-image price before submission | `gpt-image-2-rev` | Billing is fixed by output resolution |
| Multi-image reference editing | Either | Official accepts up to 16 images; reverse accepts up to 15 |

<Note>
The two lines never switch automatically: the model ID selects the line, and its own page is authoritative for the parameters it accepts.
</Note>

## What both lines share

These are identical on both lines:

| Item | Value |
|---|---|
| Endpoint | `POST /v1/tasks` (async task, see [Task system](/en/docs/task-system)) |
| Actions | `generate` text-to-image · `edit` reference-image editing (requires `image_urls`) |
| Aspect ratios | `1:1` `3:2` `2:3` `4:3` `3:4` `5:4` `4:5` `16:9` `9:16` `2:1` `1:2` `3:1` `1:3` `21:9` `9:21`, default `1:1` |
| Resolutions | `1k` `2k` `4k`, default `1k` |
| Output count | One image per request |
| Not supported | `seed` |

The differences are concentrated in **line-specific parameters** and **billing** — see "Choose a line" above.

## Unified request

```bash
curl -X POST https://www.qingbo.dev/v1/tasks \
  -H "Authorization: Bearer $WAVE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-image-2",
    "action": "generate",
    "prompt": "A blue ceramic cup on a white table in soft natural light",
    "aspect_ratio": "1:1",
    "resolution": "1k",
    "n": 1
  }'
```

A successful submission returns a `task_id`. Retrieve the result with [`GET /v1/tasks/{task_id}`](/en/api-reference/task/status), wait inline with [`Prefer: wait`](/en/api-reference/task/submit#in-request-waiting-prefer-wait), or configure a webhook.

## Related

- [GPT Image 2 Official](/en/api-reference/image/gpt-image/gpt-image-2)
- [GPT Image 2 Reverse](/en/api-reference/image/gpt-image/gpt-image-2-rev)
- [Submit Task](/en/api-reference/task/submit)
- [Query Task Status](/en/api-reference/task/status)
- [Task System](/en/docs/task-system)
