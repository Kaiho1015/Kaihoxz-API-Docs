...
<ParamField body="n" type="integer" default="1">
  Number of images to return. `grok-imagine-image`, `grok-imagine-image-quality`, `grok-imagine-1.5-rev` and `grok-imagine-1.5-edit-rev` accept `1`–`10`; `grok-imagine-image` returns one image under `edit`. `grok-imagine-image-2.0` and `grok-imagine-image-2.0-rev` accept the value `1`.
</ParamField>
...
<ParamField body="image_urls" type="string[]">
  Reference image URLs, publicly reachable. Required for `edit`: `grok-imagine-image` takes 1–5, `grok-imagine-image-quality` and `grok-imagine-image-2.0` take 1–3. `grok-imagine-1.5-rev` also accepts reference images under `generate`; `grok-imagine-image-2.0-rev` does not accept them.
...
| `grok-imagine-image-2.0` | Per request | One image per request during the initial release |
...
| `grok-imagine-image-2.0-rev` | Per request | One image per request during the initial release |
...
| `grok-imagine-image-2.0` | Priced by resolution × quality (`1k` / `2k` × `low` / `medium`); without `quality`, the resolution tier applies; each reference image is charged separately |
...
With reference images, the cost of a call is (resolution tier price + per-reference-image price × number of reference images) × number of output images. A `resolution` outside the tiers a model supports settles at its fallback price, equal to the `1k` tier.
...
```json Completed task
{
  ...
  "urls": {
    "get": "https://www.qingbo.dev/v1/tasks/task-wave1775290140a830128812",
    "cancel": "https://www.qingbo.dev/v1/tasks/task-wave1775290140a830128812/cancel"
  }
}
```
...
| `grok-imagine-1.5-rev` | `generate` | 5 | Fixed | Accepted, no extra fee |
| `grok-imagine-1.5-edit-rev` | `generate` / `edit` | Not selectable | Fixed | Accepted, no extra fee |
