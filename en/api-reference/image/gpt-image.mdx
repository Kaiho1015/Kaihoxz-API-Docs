...
<Note>
**Omitting `quality` bills at the `high` rate.** The default for `quality` is `auto`, which
lets the model pick — and the upstream charges `auto` at its top tier, so we price it the
same way. To pay less, pass `quality: "low"` or `"medium"` **explicitly**.
</Note>
