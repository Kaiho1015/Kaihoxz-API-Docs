...
<ParamField body="image_urls" type="string[]">
  Reference image URLs. Required for `edit`, up to 14; on `gemini-2.5-flash-image-preview-rev` the ceiling is left to the upstream model. Reference images carry no extra charge.
</ParamField>
...
<ParamField body="resolution" type="string" default="1k">
  Output resolution. The tiers each model offers are listed under "Available models": `1k`, `2k` and `4k` on the flagship tier, `0.5k`, `1k`, `2k` and `4k` on the fast tier, and `1k` only on the previous-generation tier.
...
| Reference images | Up to 14; on `gemini-2.5-flash-image-preview-rev` the ceiling is left to the upstream model |
...
- **Up to 14 reference images**; on `gemini-2.5-flash-image-preview-rev` the ceiling is left to the upstream model
